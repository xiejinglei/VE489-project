                ...
                int window_size, const char* file_name, const char* log_file)
{
    ...
    // record of duplicate ACKs
    int prev_ack_seq = -1;   // initialize
    int dup_ack_cnt = 0;
    ...
    while (is_all_acked(data_ack, num_packets) == 0)
    {
        ...
        if (n > 0)
        {
            ...
            // check duplicates
            if (prev_ack_seq != header.seq)
            {
                // reset
                prev_ack_seq = header.seq;
                dup_ack_cnt = 0;
            }
            else
            {
                // prev_ack_seq == header.seq: new duplicate                
                dup_ack_cnt++;
                if (dup_ack_cnt == 2)
                {
                    // resend 
                    sendto(sockfd, (void*)buf_arr[prev_ack_seq], MAX_PACKET_SIZE, 0, 
                        (const struct sockaddr *) &receiver_addr, sizeof(struct sockaddr_in));

                    struct SRHeader header_tmp = data_headers[prev_ack_seq];
                ...
                //printf("packet %d ack dup the %d time\n", prev_ack_seq, dup_ack_cnt);
                continue;
            }
