...
int get_gap_seq_helper(char **window_buffer, int window_size, int index)
{
    for (int i = index+1; i < window_size; i++)
    {
        if (window_buffer[i] != NULL)
            return 1;
    }
    return 0;
}
...
// find the first gap in sliding window (receiver)
// If no applicable return value, return -1.
int get_gap_seq(char **window_buffer, int *is_NACKed, int window_size, int window_start)
{
    //int index = 0;
    for (int i = 0; i < window_size-1; i++)
    {
        if (window_buffer[i] == NULL 
            && get_gap_seq_helper(window_buffer, window_size, i) == 1
            && is_NACKed[i] == 0)
        {
...
int run_receiver(const int port, const int window_size, const char* recv_dir, const char* log_file)
{
    ...
    int *is_NACKed = malloc(sizeof(int)*window_size);   // 1 if the gap has been NACKed
    for (int i = 0; i < window_size; i++)
        is_NACKed[i] = 0;
    ...
    while (1) 
    {
        ...
        if (n > 0) {
            ...
            if (header.flag == DATA)
            {
                ...
                {
                    // NACK?
                    int gap_seq = get_gap_seq(window_buffer_payload, is_NACKed, window_size, window_start);
                    if (gap_seq != -1)
                    {
                        struct SRHeader nack_header = {.flag = NACK, .seq = gap_seq, .len = 0, .crc = crc32(NULL, 0)};
                        sendto(sockfd, (void*) &nack_header, HEADER_SIZE, 0, (const struct sockaddr *) &si_other, 
                            sizeof(struct sockaddr_in));
                        fprintf(fp_log, "%s %d %d %d\n", FLAGS[nack_header.flag], nack_header.seq, 
                            nack_header.len, nack_header.crc);
                        fflush(fp_log);

                        // mark NACKed
                        is_NACKed[gap_seq-window_start] = 1;
                        continue;
                    }
                ...
                if (crc_check != header.crc)
                {
                    ...
                    if (gap_seq != -1)
                    {
                        struct  SRHeader nack_header = {.flag = NACK, .seq = gap_seq, .len = 0, 
                            .crc = crc32(NULL, 0)};
                        sendto(sockfd, (void*) &nack_header, HEADER_SIZE, 0, (const struct sockaddr *) &si_other, 
                            sizeof(struct sockaddr_in));
                        fprintf(fp_log, "%s %d %d %d\n", FLAGS[nack_header.flag], nack_header.seq, 
                ...
                if (header.seq == window_start)
                {
                    ...
                    for (int k = num_write; k < window_size; k++)
                    {
                        ...
                        is_NACKed[k-num_write] = is_NACKed[k]; 
                    ...
                    else
                    {
                        ...
                        int ack_syn = crc32(NULL, 0);
                        struct SRHeader ack_header = {.flag = ACK, .seq = new_window_start, .len = 0, 
                            .crc = ack_syn};
                        sendto(sockfd, (void*) &ack_header, HEADER_SIZE, 0, (const struct sockaddr *) &si_other, 
                            sizeof(struct sockaddr_in));
                        fprintf(fp_log, "%s %d %d %d\n", FLAGS[ack_header.flag], ack_header.seq, ack_header.len, 
                ...
                int window_size, const char* file_name, const char* log_file)
{
    ...
    while (is_all_acked(data_ack, num_packets) == 0)
    {
        ...
        if (n > 0)
        {
            ...
            if (header.flag == NACK)
            {
                sendto(sockfd, (void*)buf_arr[header.seq], MAX_PACKET_SIZE, 0, 
                            (const struct sockaddr *) &receiver_addr, sizeof(struct sockaddr_in));
                struct SRHeader header_tmp = data_headers[header.seq];
